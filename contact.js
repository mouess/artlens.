...
const Contact = ({ data }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const { error } = await supabase.from('contact').insert([{ name: formData.name, email: formData.email, phone: formData.phone, message: formData.message }]);;
                ...
                <div className="contact">
                    ...
                    <div className="form">
                        ...
                        <form onSubmit={handleSubmit}>
                            <input type="text" name="name" placeholder="Nom" value={formData.name} onChange={handleChange}/> <br/>
                            <input type="text" name="email" placeholder="Email" value={formData.email} onChange={handleChange}/><br/>
                            <input type="text" name="phone" placeholder="Téléphone" value={formData.phone} onChange={handleChange}/><br/>
                            <textarea name="message" placeholder="Message" value={formData.message} onChange={handleChange} ></textarea><br/>
                            <button type="submit" >Envoyer</button>
                        </form><br/>
                        {success === true && <p style={{color:'green'}}>Message envoyé avec succès !</p>}
                        {success === false && <p style={{color:'red'}}>Erreur lors de l'envoi </p>}
